...
import ru.scadarnull.gomoku.ComputerTurn;
...
public class DefaultComputerTurn implements ComputerTurn {
    ...
    protected Cell makeRandomTurn() {
        LinkedList<Cell> emptyCells = getAllEmptyCells();
    ...
    protected LinkedList<Cell> getAllEmptyCells(){
    ...
    protected Cell tryMakeTurnByRow(CellValue cellValue, int notBlankCount) {
        for (int i = 0; i < gameTable.getSize(); i++) {
            for (int j = 0; j < gameTable.getSize() - winCount - 1; j++) {
                ...
                int count = 0;
                LinkedList<Cell> inspectedCells = new LinkedList<>();
    ...
    protected Cell makeTurnToOneCellFromDataSet(LinkedList<Cell> inspectedCells) {
        Cell cell = findEmptyCellForComputerTurn(inspectedCells);
        gameTable.setValue(cell.getRowIndex(), cell.getColIndex(), CellValue.COMPUTER);
        return cell;
    }

    protected Cell findEmptyCellForComputerTurn(LinkedList<Cell> cells) {
